...
import (
	...
	"github.com/DoNewsCode/truss/deftree"
)
...
func MdService(s *deftree.ProtoService, depth int) string {
	...
	for _, meth := range s.Methods {
		reqLink := nameLink(meth.RequestType.GetName())
		resLink := nameLink(meth.ResponseType.GetName())

		rv += prindent(0, "| %v | %v | %v | %v |\n", meth.GetName(), reqLink, resLink, meth.GetDescription())
	}
	rv += "\n"
